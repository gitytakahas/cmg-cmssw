#ifndef DIPATOBJECTFACTORY_H_
#define DIPATOBJECTFACTORY_H_

#include "CMGTools/Common/interface/Factory.h"
#include "CMGTools/Common/interface/SettingTool.h"
#include "H2TauTau/Format/DiPatobject.h"
...
class DiPatObjectFactory : public cmg::Factory< cmg::DiTauObject<T, U> >, public cmg::SettingTool<std::pair<T, U>, cmg::DiTauObject<T, U> >
{

    public:

        DiPatObjectFactory(const edm::ParameterSet& ps) :             
...
template<typename T, typename U>
void cmg::DiPatObjectFactory<T, U>::set(const std::pair<T, U>& pair, const reco::LeafCandidate& met, cmg::DiTauObject<T, U>* const obj){
...
template<typename T, typename U>
void cmg::DiPatObjectFactory<T, U>::set(const std::pair<T, U>& pair, const reco::LeafCandidate& met, const cmg::METSignificance& metSig, cmg::DiTauObject<T, U>* const obj) {
  set(pair, met, obj);
  obj->metSig_ = metSig;
}

template< typename T, typename U>
typename cmg::DiPatObjectFactory<T, U>::event_ptr cmg::DiPatObjectFactory<T, U>::create(const edm::Event& iEvent, const edm::EventSetup&){
    ...
    typename cmg::DiPatObjectFactory<T, U>::event_ptr result(new collection);
      ...
      /*             if(metAvailable && metCands->size()){ */
            if(metAvailable && ! metCands->empty() ) 
            {
                if (metSigAvailable && !metSigCands->empty())
                    cmg::DiPatObjectFactory<T, U>::set(std::make_pair(cmgTmp.leg1(), cmgTmp.leg2()), metCands->at(0), metSigCands->at(0), &cmgTmp);
                else
                    cmg::DiPatObjectFactory<T, U>::set(std::make_pair(cmgTmp.leg1(), cmgTmp.leg2()), metCands->at(0), &cmgTmp);
	    /*             }    */
