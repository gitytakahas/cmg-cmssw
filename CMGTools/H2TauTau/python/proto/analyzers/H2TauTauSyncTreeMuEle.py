...
class H2TauTauSyncTreeMuEle( TreeAnalyzerNumpy ):
    ...
    def declareHandles(self):
        super(H2TauTauSyncTreeMuEle, self).declareHandles()
    ...
    def process(self, iEvent, event):
        ...
        if not event.isSignal or \
           ...
           (hasattr(event, 'leptonAccept') and not event.leptonAccept ):
            print 'SyncTree is rejected by thirdleptonveto, leptonaccept'
            return False
        ...
        if event.diLepton.leg2().charge()*event.diLepton.leg1().charge()==1:
            return False # remove as this is same sign !
...
#        fill( tr, 'mt_1', event.diLepton.mTLeg2()) # Yuta : is this true ? -> I modified Leg1 to below ...
        ...
        fill( tr, 'iso_2', leg2.relIsoAllChargedDB05())
        fill( tr, 'mva_2', leg2.mvaId()) # should be filled for e-tau: mva id
        fill( tr, 'd0_2', leg2.dxy() )
        fill( tr, 'dZ_2', leg2.dz())
        fill( tr, 'passid_2', leg2.tightId() )
        fill( tr, 'passiso_2', leg2.relIsoAllChargedDB05()<0.1 )
