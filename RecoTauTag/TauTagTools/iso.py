...
#    print 'min, max, min, max = ', x_min, x_max, y_min, y_max
    frame.GetXaxis().SetRangeUser(x_min*0.5, x_max*1.5)
    frame.GetYaxis().SetRangeUser(y_min*0.5, y_max*1.5)
    frame.Draw('same')
    ...
    save(canvas, 'plots_iso/roc_' + header)
...
#          hist = TH1F(histname, histname, 100, 0, 1)
          hist = TH1F(histname, histname, 100, 0, 5)
      
          hist.SetTitle(val['label'])
          hist.Sumw2()

          den = Double(tree.GetEntries(''))
          num = Double(tree.GetEntries(dm + ' && tau_photonsumpt_outside/tau_pt < 0.2'))
          eff = num/den
          
          tree.Draw('tau_ciso + tau_niso_weighted >> ' + hist.GetName(), dm + ' && tau_photonsumpt_outside/tau_pt < 0.2')

          hists.append([copy.deepcopy(hist), eff])


#        makeCompareVars(hists, 'Isolation', 'a.u.', True , 'compare_' + dkey + '_' + rkey + '_' + ikey, len(hists), dkey + '_' + rkey + '_' + ikey) 
