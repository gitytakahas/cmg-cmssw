...
#    _bkg_ = 0.
  ...
  roc100 = TGraph(1, numpy.asarray([efS]), numpy.asarray([efB]))

  return rocCurve, roc20, roc100
...
#        wp.SetMarkerStyle(28)
      ...
      if label.find('Standard')!=-1:
        wp100.SetMarkerStyle(mstyle)
        wp100.SetMarkerColor(2)
        wp100.SetFillColor(2)
        wp100.SetLineColor(2)
        wp100.SetLineWidth(2)
        wp100.SetMarkerSize(2)
#        wp100.SetMarkerStyle(28)
        wp100.Draw("psame")
...
#    print 'min, max, min, max = ', x_min, x_max, y_min, y_max
    ...
    nleg = TLegend(0.2,0.65,0.4,0.85)
    LegendSettings(nleg,1)

    mleg = TLegend(0.37,0.65,0.57,0.85)
    ...
    for index, i in enumerate(['loose','medium','tight']):
      ...
      gg[index] = TGraph()
      gg[index].SetPoint(0, 1,1)
      gg[index].SetMarkerSize(2)
      gg[index].SetMarkerColor(2)

      ll = ' ' + i


      if i=='loose':
        g[index].SetMarkerStyle(21)
#        mleg.AddEntry(g[index], label, '')

        gg[index].SetMarkerStyle(21)
#        nleg.AddEntry(gg[index], label, '')
#        ll += ' (Iso < 2GeV)'
      if i=='tight':
        g[index].SetMarkerStyle(23)
        gg[index].SetMarkerStyle(23)
#        ll += ' (Iso < 0.8GeV)'
      if i=='medium':
        g[index].SetMarkerStyle(20)
        gg[index].SetMarkerStyle(20)
#        ll += ' (Iso < 1GeV)'

      mleg.AddEntry(g[index], ll, 'p')
      nleg.AddEntry(gg[index], ll, 'p')
    ...
    leg2 = TLegend(0.75,0.93,0.9,0.99)
    LegendSettings(leg2, 1)
    leg2.AddEntry(hist, 'VBF / QCD', "")
    leg2.Draw()


    tex = TLatex(0.15, 0.037, 'Run-1')
    tex.SetTextFont(42)
    tex.SetTextSize(0.03)
    tex.Draw()

    tex2 = TLatex(0.3, 0.037, 'Dynamic')
    tex2.SetTextFont(42)
...
#        roc.append([rocCurve(hists[1][0], hists[0][0], hists[1][1], hists[0][1], rkey + '_' + ikey + '_' + dkey), rkey + '_'+ ikey + '_' + dkey])
