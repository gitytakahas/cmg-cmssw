...
#    leg2.Draw()
    ...
    save(c, 'plots_roc_2D_db/' + savestr)
...
#    mleg.Draw()
    ...
    save(canvas, 'plots_roc_2D_db/roc_' + header)
...
#          criteria = dm + ' && (tau_ciso + tau_niso_weighted) < ' + str(ival)
...
#        makeCompareVars(hists, '#sum p_{T}^{signal #gamma, intermediate} / tau pT', 'a.u.', True , 'compare_' + dkey + '_' + rkey + '_' + ikey, len(hists), dkey + '_' + rkey + '_' + ikey) 
      ...
      for ikey, ival in sorted(isodict.iteritems()):
        ...
        for key, val in sorted(hdict.iteritems()):
          ...
          histname = 'h_' + key + '_' + dkey + '_' + rkey + '_' + ikey + '_var'
          hist = TH1F(histname, histname, 100, 0, 1)
...
#          criteria = dm + ' && (tau_ciso + tau_niso_weighted) < ' + str(ival)
#          criteria = dm + ' && tau_ciso < ' + str(ival)
          criteria = dm
          if dkey!='Inclusive':
            criteria += '&& (tau_ciso + max(0,tau_niso_weighted - ' + isodict_vary[dkey] + ')) < ' + str(ival)
          else:
            criteria += '&& ((tau_dm_rough==0 && (tau_ciso + max(0,tau_niso_weighted - ' + isodict_vary['1p0pi0'] + ')) < ' + str(ival) +') || (tau_dm_rough==1 && (tau_ciso + max(0, tau_niso_weighted - ' + isodict_vary['1p1pi0'] + ')) < ' + str(ival) + ') || (tau_dm_rough==2 && (tau_ciso + max(0, tau_niso_weighted - ' + isodict_vary['3p'] + ')) < ' + str(ival) + '))'
        ...
        roc.append([rocCurve(hists[1][0], hists[0][0], hists[1][1], hists[0][1], rkey + '_' + ikey + '_' + dkey + '_vary'), 'Optimized'])
