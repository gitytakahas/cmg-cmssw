...
if __name__ == '__main__':
    ...
    yvardict = {
        'nIso_raw':{'tree':'per_tau','var':'tau_niso_weighted', 'nbin':2000, 'xmin':0., 'xmax':100, 'title':'Neutral weighted Iso. (GeV)', 'sel':'tau_dm!=-1 && tau_pt > 20 && abs(tau_eta) < 2.3', 'name':'VBF_weighted_niso_overlay_pt'},
        }
    ...
    for vkey, ivar in yvardict.iteritems():
        ...
        for dkey, dm in ddict.iteritems():
            ...
            _hist_ = TH2F('h_effp_' + dkey, 'h_effp_' + dkey, len(binning)-1, array('d',binning), ivar['nbin'], ivar['xmin'], ivar['xmax'])


            dname = ivar['var'] + ':tau_genpt >> ' + _hist_.GetName()
            tree.Draw(dname, ivar['sel'] + '&&' + dm['cut'])

            hist = _hist_.ProfileX()

            hist.GetXaxis().SetTitle('gen tau p_{T}^{vis} (GeV)')
