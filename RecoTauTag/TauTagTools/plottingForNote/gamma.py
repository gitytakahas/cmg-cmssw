...
#    hist_edge.SetMarkerColor(kRed)
    hist_edge.SetLineColor(1)
    hist_edge.SetMarkerColor(1)
...
if __name__ == '__main__':
    ...
    tfile = TFile('Myroot_singleTauGun_envelope.root')
    tree = tfile.Get('per_photon')

    nbin = 1000

    makeEffPlotsVars(tree, 'photon_pt', 'abs(photon_deta)', 'photon_gendm>=0 && photon_pt > 0.5 && photon_dm_rough !=-1', nbin, 0., 1, 'p_{T}^{e/#gamma} (GeV)', '#Delta#eta(#tau, e/#gamma)', 'Inclusive, 95% envelope', 'strip_pt_gamma_deta_inclusive_95', 'pt', 0.95)
    makeEffPlotsVars(tree, 'photon_pt', 'abs(photon_dphi)', 'photon_gendm>=0 && photon_pt > 0.5 && photon_dm_rough !=-1', nbin, 0., 1, 'p_{T}^{e/#gamma} (GeV)', '#Delta#phi(#tau, e/#gamma)', 'Inclusive, 95% envelope', 'strip_pt_gamma_dphi_inclusive_95', 'pt', 0.95)
