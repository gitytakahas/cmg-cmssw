...
#      wp.Draw("psame")
      ...
      wp2.SetMarkerSize(2)
      wp2.SetMarkerStyle(29)
#      wp2.Draw("psame")
      
      wp3.SetMarkerColor(1)
      wp3.SetMarkerSize(2)
      wp3.SetMarkerStyle(30)
#      wp3.Draw("psame")
...
#    frame.GetYaxis().SetRangeUser(y_min, y_max)
    ...
    for index, i in enumerate(['loose','medium','tight']):
      ...
      if i=='loose':
        g[index].SetMarkerStyle(28)
#        ll += ' (Iso < 2GeV)'
        ll = '10%'
      if i=='tight':
        g[index].SetMarkerStyle(29)
        ll += ' (Iso < 0.8GeV)'
        ll = '15%'
      if i=='medium':
        g[index].SetMarkerStyle(30)
#        ll += ' (Iso < 1GeV)'
        ll = '20%'
...
if __name__ == '__main__':
  ...
  for pkey, pval in sorted(ptdict.iteritems()):
      ...
      for rkey, rval in sorted(runtype.iteritems()):
        ...
        for ikey, ival in sorted(isodict.iteritems()):
          ...
          for key, val in sorted(hdict.iteritems()):
            ...
            criteria = dm + ' && (tau_ciso + tau_niso_weighted) < ' + str(ival) + ' && tau_pt > ' + pval['low']  + ' && tau_pt < ' + pval['high'] + ' && abs(tau_eta) < 2.3'
          ...
          roc.append([rocCurve(hists[1][0], hists[0][0], hists[1][1], hists[0][1], rkey + '_' + ikey + '_' + dkey + '_' + pkey), rval['type']])


          if rkey!='dynamic95': continue
          ...
          for key, val in sorted(hdict.iteritems()):
            ...
            histname = 'g_' + key + '_' + dkey + '_' + rkey + '_' + ikey
            hist = TH1F(histname, histname, 100, 0, 1)
      
            hist.SetTitle(val['label'] + ', pT^{alt}')
            hist.Sumw2()
            
            den = Double(tree.GetEntries(''))
            
            criteria = dm + ' && (tau_ciso + tau_niso_weighted) < ' + str(ival) + ' && tau_pt > ' + pval['low']  + ' && tau_pt < ' + pval['high'] + ' && abs(tau_eta) < 2.3 && (tau_pt - tau_photonsumpt_outside) > 20'
            num = Double(tree.GetEntries(criteria))
            eff = num/den
          ...
          roc.append([rocCurve(hists[1][0], hists[0][0], hists[1][1], hists[0][1], 'tune_' + rkey + '_' + ikey + '_' + dkey + '_' + pkey), rval['type']])

      overlay(roc, dkey + '_' + pval['low'] + '_' + pval['high'])
