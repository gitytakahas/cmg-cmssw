...
#    tex.Draw()
    ...
    save(c, 'plots_isolation_vbf/' + header)
...
#        hist.SetMinimum(ymin*1.2)
        ...
        lname = hist.GetTitle() + '(Mean =' + RV(hist.GetMean()) + ', RMS =' + RV(hist.GetRMS()) + ')'
...
#    line.Draw()
    ...
    save(c, 'plots_isolation_vbf/' + savestr)
...
#        'standard':{'file':'Myroot_standard.root','label':'Standard (run-1)'},
        'standard':{'file':'Myroot_run1.root','label':'Standard (run-1)'},
...
#    ddict = {'1p0pi0':'tau_dm_rough==0', '1p1pi0':'tau_dm_rough==1', '3p':'tau_dm_rough==2', 'Inclusive':'1'}
    ...
    vardict = {
        ...
        'nphoton_ratio_ciso':{'tree':'per_tau','var':'tau_nphoton_inside / (tau_nphoton_inside + tau_nphoton_outside)', 'nbin':20, 'xmin':0, 'xmax':1, 'title':'fraction of N_{#gamma} after cIso < 2', 'sel':'tau_dm!=-1 && tau_ciso < 2'},
        ...
        'sumpt_total':{'tree':'per_tau','var':'tau_photonsumpt_inside + tau_photonsumpt_outside', 'nbin':30, 'xmin':0, 'xmax':150, 'title':'#sum p_{T}^{#gamma}', 'sel':'tau_dm!=-1'},
        'sumpt_ratio':{'tree':'per_tau','var':'tau_photonsumpt_outside/tau_pt', 'nbin':30, 'xmin':0, 'xmax':2, 'title':'#sum p_{T}^{#gamma, outside} / tau pT', 'sel':'tau_dm!=-1'},
        'tau_gen_nphoton_inside':{'tree':'per_tau','var':'tau_gen_nphoton_inside', 'nbin':20, 'xmin':0, 'xmax':20, 'title':'#gen photon inside signal cone', 'sel':'tau_dm!=-1'},
        'tau_gen_nphoton_outside':{'tree':'per_tau','var':'tau_gen_nphoton_outside', 'nbin':20, 'xmin':0, 'xmax':20, 'title':'#gen photon outside signal cone', 'sel':'tau_dm!=-1'},
        'tau_gen_sumpt_inside':{'tree':'per_tau','var':'tau_gen_sumpt_inside', 'nbin':30, 'xmin':0, 'xmax':100, 'title':'#sum p_{T}^{#gamma, gen} inside signal cone', 'sel':'tau_dm!=-1'},
        'tau_gen_sumpt_outside':{'tree':'per_tau','var':'tau_gen_sumpt_outside', 'nbin':30, 'xmin':0, 'xmax':100, 'title':'#sum p_{T}^{#gamma, gen} outside signal cone', 'sel':'tau_dm!=-1'},
        'ptratio':{'tree':'per_tau','var':'tau_pt/tau_genpt', 'nbin':50, 'xmin':0, 'xmax':2, 'title':'tau p_{T}/tau gen p_{T}', 'sel':'tau_dm!=-1'},
        ...
        'reco_dm':{'tree':'per_tau','var':'tau_dm', 'nbin':12, 'xmin':-1, 'xmax':11, 'title':'Reco. decaymode', 'sel':'1'},
        ...
        'reco_gendm_niso':{'tree':'per_tau','var':'tau_gendm', 'nbin':12, 'xmin':-1, 'xmax':11, 'title':'Gen. decaymode (cmb iso < 2)', 'sel':'(tau_ciso + tau_niso_weighted) < 2'},
        'reco_dm_rough':{'tree':'per_tau','var':'tau_dm_rough', 'nbin':4, 'xmin':-1, 'xmax':3, 'title':'Reco. decaymode (simplified)', 'sel':'1'},
        'reco_dm_isdm':{'tree':'per_tau','var':'tau_dm !=-1', 'nbin':2, 'xmin':0, 'xmax':2, 'title':'Reco. isDecaymode', 'sel':'1'},
        'tau_ciso':{'tree':'per_tau','var':'tau_ciso', 'nbin':50, 'xmin':0, 'xmax':10, 'title':'Tau charged isolation', 'sel':'1'},
        'tau_niso':{'tree':'per_tau','var':'tau_niso', 'nbin':50, 'xmin':0, 'xmax':20, 'title':'Tau neutral isolation', 'sel':'1'},
        'tau_puiso':{'tree':'per_tau','var':'tau_puiso', 'nbin':50, 'xmin':0, 'xmax':50, 'title':'Tau PU isolation', 'sel':'1'},
        }
    ...
    yvardict = {
        ...
        'cIso_raw':{'tree':'per_tau','var':'tau_ciso', 'nbin':2000, 'xmin':0., 'xmax':500, 'title':'cIso', 'sel':'tau_dm!=-1 && tau_pt > 20 && abs(tau_eta) < 2.3', 'name':'dm_raw_cIso'},
        'nIso_raw':{'tree':'per_tau','var':'tau_niso', 'nbin':2000, 'xmin':0., 'xmax':500, 'title':'nIso', 'sel':'tau_dm!=-1 && tau_pt > 20 && abs(tau_eta) < 2.3', 'name':'dm_raw_nIso'},
        'nIso_raw_weight':{'tree':'per_tau','var':'tau_niso_weighted', 'nbin':2000, 'xmin':0., 'xmax':500, 'title':'nIso weight', 'sel':'tau_dm!=-1 && tau_pt > 20 && abs(tau_eta) < 2.3', 'name':'dm_raw_nIso_weight'},
        'cIso_nIso_raw_weight':{'tree':'per_tau','var':'tau_ciso + tau_niso_weighted', 'nbin':2000, 'xmin':0., 'xmax':500, 'title':'cIso + nIso weight', 'sel':'tau_dm!=-1 && tau_pt > 20 && abs(tau_eta) < 2.3', 'name':'dm_raw_cIso_nIso_weight'},
        }
    ...
    for vkey, ivar in yvardict.iteritems():
        for dkey, dm in ddict.iteritems():
            ...
            for key, val in sorted(hdict.iteritems()):
                ...
                hists.append(makeEffPlotsVars(tree, 'tau_genpt', ivar['var'], ivar['sel'] + ' && ' + dm, 30, 0, 300, ivar['nbin'], ivar['xmin'], ivar['xmax'], 'gen tau p_{T}^{vis} (GeV)', ivar['title'], dkey + ', ' + val['label'], vkey + '_' + dkey + '_' + key + '_fake', vkey + '_' + dkey + '_' + key))

            overlay(hists, 'eff_taupt_' + ivar['name'] + '_' + dkey, dkey)
    ...
    for vkey, ivar in yvardict.iteritems():
        for dkey, dm in ddict.iteritems():
            ...
            for key, val in sorted(hdict.iteritems()):
                ...
                hists.append(makeEffPlotsVars(tree, 'tau_pt', ivar['var'], ivar['sel'] + ' && ' + dm, 30, 0, 300, ivar['nbin'], ivar['xmin'], ivar['xmax'], 'reco. tau p_{T}^{vis} (GeV)', ivar['title'], dkey + ', ' + val['label'], 'taupt_' + vkey + '_' + dkey + '_' + key + '_fake', 'taupt_' + vkey + '_' + dkey + '_' + key))


            overlay(hists, 'taupt_' + ivar['name'] + '_' + dkey, dkey)
            ...
            for key, val in sorted(hdict.iteritems()):
                ...
                hists.append(makeEffPlotsVars(tree, 'tau_geneta', ivar['var'], ivar['sel'] + ' && ' + dm, 20, -2.5, 2.5, ivar['nbin'], ivar['xmin'], ivar['xmax'], 'gen tau #eta^{vis}', ivar['title'], dkey + ', ' + val['label'], 'taueta_' + vkey + '_' + dkey + '_' + key + '_fake', 'taueta_' + vkey + '_' + dkey + '_' + key, 'eta'))


            overlay(hists, 'eff_taueta_' + ivar['name'] + '_' + dkey, dkey)
            ...
            for key, val in sorted(hdict.iteritems()):
                ...
                hists.append(makeEffPlotsVars(tree, 'tau_gvertex', ivar['var'], ivar['sel'] + ' && ' + dm, 30, 0, 30, ivar['nbin'], ivar['xmin'], ivar['xmax'], '# of good vertices', ivar['title'], dkey + ', ' + val['label'], 'tau_nvtx_' + vkey + '_' + dkey + '_' + key + '_fake', 'tau_nvtx_' + vkey + '_' + dkey + '_' + key, 'nvtx'))


            overlay(hists, 'eff_tau_nvtx_' + ivar['name'] + '_' + dkey, dkey)
